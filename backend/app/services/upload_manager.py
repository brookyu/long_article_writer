...
class UploadManager:
    ...
    async def create_upload_job(
        ...
        # Debug logging
        logger.info(f"🔍 create_upload_job called:")
        logger.info(f"  upload_type: {upload_type}")
        logger.info(f"  file_paths: {file_paths}")
        logger.info(f"  file_paths length: {len(file_paths) if file_paths else 0}")
        ...
        try:
            ...
            if upload_type == "folder" and folder_path:
                ...
                logger.info(f"Setting file_list for job {job_id}: {len(file_paths)} files")
            ...
            # Debug: Check if file_list was saved correctly
            logger.info(f"After commit - job {job_id} file_list length: {len(upload_job.file_list) if upload_job.file_list else 0}")
        ...
        try:
            ...
            return {
                ...
                "progress": {
                    ...
                    "percentage": (job.successful_files / job.total_files * 100) if job.total_files else 0
                },
                "timestamps": {
        ...
        try:
            ...
            for job in jobs:
                job_data = {
                    ...
                    "file_list": job.file_list or [],  # Add the missing file_list field!
                    "upload_path": job.upload_path,     # Also add upload_path for completeness
                    "errors": job.error_log or []
                }
